...
public class Car extends Vehicle {
	...
	public void refuel(int amountOfGas) {
		...
		int gas = this.gasRemained + amountOfGas;

		if (gas <= this.maxGasAmountInTank) {
			this.gasRemained = this.maxGasAmountInTank;
		} else {
			this.gasRemained = gas;
		}

		System.out.println("Refueled! Gas remaining: " + this.gasRemained);
	}
	...
	public void run(int hour) {
		...
		if (this.gasRemained <= 0) {
			System.out.println("Gas out! Please refuel!");
			return;
		}
		int gasConsumed = hour * this.gasConsumedPerHour;

		if (gasConsumed > this.gasRemained) {
			System.out.println("Oops out of fuel, add fuel!");
